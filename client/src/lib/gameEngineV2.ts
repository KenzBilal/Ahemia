...
export class GameEngineV2 {
  ...
  private invincibilityEnd: Record<string, number> = {};
  private respawnQueue: Array<{ playerId: string; respawnTime: number }> = [];
  ...
  constructor(canvas: HTMLCanvasElement, gameState: GameState, localPlayerId: string) {
    ...
    Object.keys(gameState.players).forEach((id) => {
      ...
      this.invincibilityEnd[id] = Date.now() + 3000; // 3 seconds invincibility
    });
  }
  ...
  updatePlayerMovement(playerId: string, moveX: number, moveY: number, jetpackActive: boolean = false) {
    ...
    // Jetpack thrust
    if (jetpackActive && this.jetpackFuel[playerId] > 0) {
      player.vy -= JETPACK_THRUST;
      this.jetpackFuel[playerId] -= JETPACK_FUEL_DRAIN;
  ...
  private addParticle(x: number, y: number, vx: number, vy: number, color: string) {
    this.particles.push({
      x,
      y,
      vx,
  ...
  update(deltaTime: number) {
    ...
    Object.values(this.gameState.players).forEach((player) => {
      ...
      if (player.y + PLAYER_HEIGHT / 2 >= GROUND_LEVEL) {
        ...
        if (this.jetpackFuel[player.id] < JETPACK_FUEL_MAX) {
          this.jetpackFuel[player.id] = Math.min(
            this.jetpackFuel[player.id] + JETPACK_FUEL_REGEN,
            JETPACK_FUEL_MAX
          );
        }
      }

      // Lava map instant death
      if (this.gameState.settings.map === "lava" && player.y > GROUND_LEVEL) {
      ...
      if (player.x + PLAYER_WIDTH / 2 > this.mapConfig.width)
        player.x = this.mapConfig.width - PLAYER_WIDTH / 2;
      ...
      if (player.isInvincible && now > this.invincibilityEnd[player.id]) {
    ...
    this.gameState.bullets.forEach((bullet) => {
      Object.values(this.gameState.players).forEach((player) => {
        ...
        if (distance < PLAYER_WIDTH / 2 + 5) {
          ...
          // Add hit particle
          this.addParticle(bullet.x, bullet.y, (Math.random() - 0.5) * 4, (Math.random() - 0.5) * 4, "#FF0000");
          ...
          if (player.health <= 0) {
            ...
            this.respawnQueue.push({
              playerId: player.id,
              respawnTime: now + this.gameState.settings.respawnTime,
            });
          }
    ...
    // Process respawns
    this.respawnQueue = this.respawnQueue.filter((respawn) => {
      if (now >= respawn.respawnTime) {
        const player = this.gameState.players[respawn.playerId];
        if (player) {
          const mapConfig = MAP_CONFIGS[this.gameState.settings.map];
          ...
          this.invincibilityEnd[player.id] = now + 3000;
  ...
  private drawPlayer(player: PlayerState) {
    ...
    this.ctx.fillRect(
      -PLAYER_WIDTH / 2,
      -PLAYER_HEIGHT / 2 - 10,
      (PLAYER_WIDTH * player.health) / player.maxHealth,
      5
    );
